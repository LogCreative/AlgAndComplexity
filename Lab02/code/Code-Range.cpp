...
int binary_search_for_m(vector<long>& sum, long sum_i, int LOWER, int low, int high) {
	...
	*/
	...
	int mid = (low + high)/2;
	if (sum[mid] - sum_i >= LOWER) return binary_search_for_m(sum,sum_i,LOWER,low,mid);
	else return binary_search_for_m(sum,sum_i,LOWER,mid+1,high);
}
...
int binary_search_for_n(vector<long>& sum, long sum_i, int UPPER, int low, int high) {
	...
	*/
	if (low == high) return high;
	int mid = (low + high)/2;
	if (sum[mid] - sum_i > UPPER) return binary_search_for_n(sum,sum_i,UPPER,low,mid);
	else return binary_search_for_n(sum,sum_i,UPPER,mid+1,high);
}
